...
COOLDOWN=180          # seconds to wait after an auto-reset before resuming checks
    ...
    while true; do
        ...
        if check_internet; then
            ...
            if [ "$fail_count" -ge "$FAIL_THRESHOLD" ]; then
                echo "$(date): internet dianggap mati, menjalankan reset jaringan otomatis"
                "$NOTIFY" "Internet Terputus" "Terdeteksi mati, menjalankan Reset Jaringan otomatis..." 2>/dev/null
